  ...
  methods: {
    ...
    init() {
      ...
      for (let i = 0; i < arrI.length; i++) {
        let obj = arrI[i];
        let h = obj.dataArray && obj.len> 1 ? this.yAxis.transStyle.column * obj.len + this.yAxis.transStyle.interval * (obj.len + 1) : this.yAxis.transStyle.height
        let oLen = arrI[i - 1] && arrI[i - 1].len
        if(i == 0) {
          newHeight = 0
        } else {
          if(oLen == 0 || oLen == 1) {
        ...
        let topY = this.topY(i, newHeight) + yAxis.boundaryGap[0];
        ...
        // 时间冲突
        if(obj.dataArray.length> 1){
          this.confilct(rowGroup,obj.dataArray, h)
        }
        ...
        if (arrJ.length) {
          ...
          for (let j = 0; j < arrJ.length; j++) {
            ...
            if (_.get(arrJ[j], s) && _.get(arrJ[j], e)) {
              ...
              // rowLineMask = rowGroup.querySelector(".rowLineMask");
              rowGroup.appendChild(colGroup);
              // rowGroup.appendChild(rowLineMask);
              ...
              let blockH;
              let jLen = arrJ.length

              if(jLen > 1 ) {
                let oY = y
                blockH = this.yAxis.transStyle.column;
                if(j == 0) y = j * this.yAxis.transStyle.column
                else {
                  if(arrJ[j-1] && new Date(_.get(arrJ[j-1], e)).getTime() < new Date(_.get(arrJ[j], s)).getTime()){
                    y = (j - 1) * (this.yAxis.transStyle.column + this.yAxis.transStyle.interval)
                  }else {
                    y = oY + (this.yAxis.transStyle.column + this.yAxis.transStyle.interval)
                  }

                }
                oY = y
              } else {
                blockH = h - this.yAxis.transStyle.interval * 2;
                y = 0
              }
              ...
              // .attr('stroke-width', 1)
              /* 根据状态判断添加哪些形状 */
              this.drawPath(colGroup, 0, y, width, blockH, arrJ[j], colBlock);

              let fontY = y
              let chargeY = this.yAxis.transStyle.column + this.yAxis.transStyle.interval
    ...
    confilct(rowGroup,arrJ, height){
      for(let k = 0;k< arrJ.length;k++) {
        let start, end;
        start = arrJ[k].ats
        end = arrJ[k].ete
        for (let i= k + 1;i< arrJ.length;i++) {
          let eT = arrJ[i].ete
          let sT = arrJ[i].ats
          if( new Date(end).getTime() < new Date(sT).getTime() ){

          } else if(new Date(end).getTime() > new Date(sT).getTime() && new Date(end).getTime() < new Date(eT).getTime()){
            this.drawRect(rowGroup, sT, end, height)
          } else if(new Date(end).getTime() > new Date(sT).getTime() && ( new Date(end).getTime() > new Date(eT).getTime() ||  new Date(end).getTime() == new Date(eT).getTime())) {
            this.drawRect(rowGroup, sT, eT, height)
          }
        }
      }
    },
    drawRect(rowGroup,sT, eT, height){
      let now = new Date(this.now.substr(0, 16) + ":00");
      // 甘特图开始时间
      let sTime = new Date(this.start.substr(0, 16) + ":00");
      // 甘特图结束时间
      let eTime = new Date(
        sTime.getTime() + this.xAxis.spanning * this.xAxis.timestempUnit
      );
      // 甘特条开始时间
      let lTime = new Date(sT.substr(0, 16) + ":00");
      if (lTime < sTime) {
        lTime = sTime;
      } else if (lTime > eTime) {
        lTime = eTime;
      ...
      let rTime = new Date(eT.substr(0, 16) + ":00");

      let li =
        (lTime.getTime() - sTime.getTime()) / this.xAxis.timestempUnit;
      let lX = this.lineX(li);
      let ri =
        (rTime.getTime() - sTime.getTime()) / this.xAxis.timestempUnit;
      let rX = this.lineX(ri);
      let width = rX - lX;
      this.$d3
        .select(rowGroup)
