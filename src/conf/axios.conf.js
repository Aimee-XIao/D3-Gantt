...
const forwardURLAiis = '/aiis'
const forwardURLMqtt = '/mqtt'
// const forwardURL = ''
// const forwardOtherURL = ''
// const forwardURLStat = ''
// const forwardURLAiis = ''
// const forwardURLMqtt = ''
...
// 超时自动退出
let timeoutFunc = function (id) {
  let gId = localStorage.getItem('customTimeoutTimerId')
  if (gId === id) {
    console.warn('logout')
    localStorage.removeItem('customTimeoutTimerId')
    router.push({path: '/'})
  }
}
...
function response (response) {
  ...
  if (response.data.code == -1 && response.data.msg == 'diffToken') {
    ...
    let customTimer = localStorage.getItem('customTimeoutTimer')
    let id = localStorage.getItem('customTimeoutTimerId')
    id && customTimer && window.clearTimeout(Number(customTimer))
    id = createGuid()
    customTimer = window.setTimeout(timeoutFunc.bind(undefined, id), 5000)
    localStorage.setItem('customTimeoutTimer', customTimer)
    localStorage.setItem('customTimeoutTimerId', id)
    return response
  }
}
    ...
    if (error.hasOwnProperty('response')) {
      if (error.response.status != 400) {
        ...
        vue.$message({
          ...
          dangerouslyUseHTMLString: true,
          message: `
            <div class="msg-box-cont show-warning">
              <div class="msg-deco"><img :src="require('@img/title_deco.png')" /></div>
...
const axiosReqAiis = axios.create({
  baseURL: forwardURLAiis + '/aoms-aiis',
  headers: {
    'Content-Type': 'application/json;charset=UTF-8',
    'Access-Control-Allow-Origin': '*',
    'Access-Control-Allow-Credentials': 'true',
...
axiosReqAiis.all = axios.all
axiosReqAiis.spread = axios.spread
axiosReqAiis.interceptors.request.use(request, requestError)
axiosReqAiis.interceptors.response.use(response, responseErr)

const axiosReqMqtt = axios.create({
  baseURL: forwardURLMqtt + '/imsp',
  headers: {
    'Content-Type': 'application/json;charset=UTF-8',
    'Access-Control-Allow-Origin': '*',
    'Access-Control-Allow-Credentials': 'true',
...
axiosReqMqtt.all = axios.all
axiosReqMqtt.spread = axios.spread
axiosReqMqtt.interceptors.request.use(request, requestError)
axiosReqMqtt.interceptors.response.use(response, responseErr)
...
uploadAiis.interceptors.request.use(request, requestError)
uploadAiis.interceptors.response.use(response, responseErr)

export {removePending, axiosReq, axiosReqStat, axiosReqAiis, axiosReqMqtt, axiosNoneReq, axiosOtherReq, axiosDfs, upload, uploadAiis}
